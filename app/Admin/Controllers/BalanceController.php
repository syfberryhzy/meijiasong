...
class BalanceController extends Controller
{
    ...
    {
        return Admin::grid(Balance::class, function (Grid $grid) {
            ...
            $grid->type('方式')->display(function ($type) {
                if ($type == 1) {
                    return  '<span style="color:rgb(92, 184, 92);">充值</span>';
                } elseif ($type == 2) {
                    return  '<span style="color:rgb(217, 83, 79);">消费</span>';
                } elseif ($type == 3) {
    ...
    public function gridSearch($grid)
    {
        $grid->filter(function ($filter) {
            ...
            $filter->equal('type', '方式')->select([1 => '充值', 2 => '消费', 3 => '退回']);
    ...
    {
        return Admin::form(Balance::class, function (Form $form) {
            $form->display('id', 'ID');
